...
async def _export_full_message(
    ...
    # Fork: signed-URL delivery. When signed attachment URLs are enabled, skip the
    # export write entirely and hand back a short-lived signed URL — the
    # /attachments/signed (or short /dl/) route re-fetches the complete message from
    # Gmail and streams it on demand (the "gmail_message" fetcher in
    # core.signed_download). Nothing lands on disk and nothing streams through the
    # model context, which also makes the export available in stateless mode
    # (upstream's fallback there is inline delivery, kept below).
    ...
    if signed_attachment_urls_enabled():
        ...
        fmt = {"raw": "eml", "html": "html", "text": "txt"}[body_format]
        signed_extension = {"eml": ".eml", "html": ".html", "txt": ".txt"}[fmt]
        signed_mime = {
            "eml": "message/rfc822",
            "html": "text/html",
            "txt": "text/plain",
        ...
        try:
            ...
            logger.debug(
                f"[_export_full_message] Could not recover credentials: {cred_exc}"
            )
        ...
        if creds and eff_ttl > 0:
            download_url = await build_download_url(
                ...
                ref={"mid": message_id, "fmt": fmt},
                filename=f"{subject[:80]}{signed_extension}",
            ...
            try:
                ...
                logger.debug(
                    f"[_export_full_message] Could not pre-cache credentials: {cache_exc}"
                )
            result_lines = _format_message_header_lines(headers)
            result_lines.append("\n--- FULL MESSAGE EXPORT (signed URL) ---")
            result_lines.append(f"Format: {fmt}")
            ...
            logger.info(
                "[_export_full_message] Returning signed streaming URL (no export write)"
            )
            return "\n".join(result_lines)
        logger.info(
            "[_export_full_message] Signed URLs enabled but no recoverable credentials "
            "(or token too near expiry); falling back to the standard export path."
        )
